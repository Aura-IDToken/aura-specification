...
- **DQ-006 status of record** reconciled from CLOSED to **OPEN** in `closures/DQ-006_CLOSURE_PACKAGE.md`, now the single authoritative closure record; four duplicate records marked SUPERSEDED. Cross-language byte/SHA-256/leaf equality on CANONICAL-001 remains PASS; closure withheld pending a JCS-discriminating cross-language vector, evidence reachability, and ratification
- **DQ-006 CLOSED** (2026-08-20, closure integration). Residual R1 was closed by execution: **CANONICAL-002**, a JCS-discriminating fixture, was added to the corpus and executed on both frozen engines, producing identical canonical bytes (655 B), SHA-256 and RFC 6962 leaf. A negative control that substitutes sorted-JSON output for one implementation is inert on CANONICAL-001 and rejected on CANONICAL-002, so the cross-language result now evidences conformance to RFC 8785 rather than only agreement. Ratification recorded. CONF-003 verdict PARTIAL → PASS on the two-fixture corpus. Carried to the release gate: merging the evidence to the reference repositories' default branches (C-1), disposing of two superseded RI-RS branches (C-2), and renaming the RI-side `CROSS-LANGUAGE-002` identifier, which collides with the DQ-002 Merkle gate (C-3). Both JCS engines remain conformance-scoped; no production runtime change
